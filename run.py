...
with open(job_file, 'w') as f:
    ...
    for gpu, data in gpu_dct.items():
        for [resolution, input_images] in data:
            ...
            elif isinstance(input_images, list) and os.path.isfile(input_images[0]):
                line += f" python infv2.py"
                line += f" --path_model {path_model}"
                line += f" --images {input_images}"
...
#not tested should run on single gpu
# def nnunetv2_inference(model_path,  # path to trained models
#                         dir_input_images,  # directory with input images
#                         dir_output_seg,  # output directory
#                         resolutions,  # 3d_fullres
#                         save_probs=True,
#                         version=2,
#                         run=False,
#                         task=None
